...
// import { meladai } from '@/config/melad-ai-providers';
// import { generateObject } from 'ai';
// import { z } from 'zod';

// const currentDate = new Date().toLocaleDateString("en-US", { year: "numeric", month: "short", day: "2-digit", weekday: "short" });

// export async function suggestQuestions(history: any[]) {

//   console.log('suggestQuestions action started', { historyLength: history.length });
//   console.log('Last message in history:', history[history.length - 1]);
//   console.log(history);
//   // for the followups
//   const { object } = await generateObject({
//     model: meladai.languageModel("melad-follow"),
//     temperature: 0.1,
//     maxTokens: 99999,
//     topP: 0.9,
//     topK: 7,
//     system:
//       `You are a search engine query/questions generator. You must create Exactly 3 relevent open ended questions for based on the message history provided.
// The questions should encourage further discussion while maintaining the whole context. Limit it to 5-10 words per question.
// Always put the user input's context is some way so that the next search knows what to search for exactly.
// Try to stick to the context of the conversation and avoid questions that are too general or too specific.
// For weather sent to you, always generate questions that are about news, sports, or other topics that are not related to the weather.
// For programming, always generate questions that are about the algorithms, data structures, or other topics that are related to it or an improvement of the question.
// For location, always generate questions that are about the culture, history, or other topics that are related to the location.
// Do not use pronouns like he, she, him, his, her, etc. as they blur the context. Always use the proper nouns from the context.`,
//     messages: history,
//     schema: z.object({
...
//   console.log('Generated questions:', object.questions);
//   return {
...
//   console.log('generateSpeech action started', { textLength: text.length });
...
//   ] as const,
//   chat: [] as const,   // chat: ['memory_manager'] as const,
...
//   search: `
//   You are an AI web search engine called Melad, designed to help users find information on the internet with no unnecessary chatter and more focus on the content.
...
//   - After the first message or search, if the user asks something other than doing the searches or responds with a feedback, Discuss in natural language.
...
//   chat: `
//   - You are Melad, a digital friend, AI agent, and assistant that helps users with anything possible, including remembering things for them.
//   - Today's date is ${currentDate}.
  
//   ### Core Responsibilities:
//   1. Talk to the user in a friendly and engaging manner.
//   2. If the user shares something with you, remember it using the memory tool and use it to help them in the future.
//   3. If the user asks you to search for something or something about themselves, use the memory tool to search for it.
//   4. Do not talk about the memory results directly in the response, if you do retrieve something, just talk about it in a natural language.
//   5. Always confirm successful memory operations (saving/retrieving).
//   6. Handle memory updates and deletions carefully if implemented.
//   7. Maintain a friendly, personal tone.
//   8. Always save the memory the user asks you to save.
...
//   Response Format:
//   - Start with a introduction and then do sections and finally a conclusion
...
// const groupPrompts = {
//   search: `${groupResponseGuidelines.search}\n\n${groupToolInstructions.search}`,
//   chat: `${groupResponseGuidelines.chat}\n\n${groupToolInstructions.chat}`,
...
//   yoink: `${groupResponseGuidelines.yoink}\n\n${groupToolInstructions.yoink}`,
// } as const;

// export async function getGroupConfig(groupId: SearchGroupId = 'search') {
//   // "use server";
//   console.log('getGroupConfig action started', { groupId });
...
//   console.log('Group config retrieved', { 
//     hasTools: !!tools, 
//     promptLength: systemPrompt?.length,
//     hasToolInstructions: !!toolInstructions,
//     hasResponseGuidelines: !!responseGuidelines
//   });
